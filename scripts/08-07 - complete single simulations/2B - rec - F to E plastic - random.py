...
# Normal experiment
# baseline_drift_mag = np.full((n_seeds, n_days + 1, N), np.nan)
# baseline_drift_rate = np.full((n_seeds, n_days, N), np.nan)
# cno_drift_mag = np.full((n_seeds, n_days + 1, N), np.nan)
# cno_drift_rate = np.full((n_seeds, n_days, N), np.nan)


def generate(seeds, hebb_k, eta_k,
                n_days, N, cno_inh_input_scale, baseline_inh_input_scale,
                save_loc_sim):
    for seed_idx, seed in tqdm(enumerate(seeds), desc="Generating simulations", total=len(seeds)):
...
if __name__ == "__main__":
    ...
    save_loc_general = "../../results/08-07 - complete single simulations/2B - rec - F to E plastic - random/"
