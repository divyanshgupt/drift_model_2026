...
save_loc_general = "../../results/06-29 - Feedforward complete analysis/1 - FF - cotuned - Hebb vs stochastic/"
...
for i, hebb_k in enumerate(tqdm(hebb_k_range)):
    for j, eta_k in enumerate(tqdm(eta_k_range)):
        ...
        ## Baseline
        net = FeedForward(inh='on', inh_type='co-tuned', inh_scale=1.0, hebb_scaling=hebb_k, rand_scaling=1, n_days=100)
        ...
        ## CNO
        net_CNO = FeedForward(inh='on', inh_type='co-tuned', inh_scale=0.6, hebb_scaling=hebb_k, rand_scaling=1, n_days=100)
