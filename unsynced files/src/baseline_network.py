...
class BaselineNetwork():
    ...
    def __init__(self, inh_type="co-tuned", E_to_E="off",
                  ...
                  inh_time_varying="off", inh_mod_scale=0.5, inh_mod_start=7, inh_mod_end=14,
    ...
    def setup_weights(self):
        ...
        self.w_if *= self.inh_scale # scale I input weights by inh scale to ensure overall inhibition is scaled appropriately
    ...
    def run(self):
        ...
        for stim_idx in tqdm(range(self.n_stim_total)):
            ...
            if (stim_idx + 1) % self.n_stim_per_day == 0:
                PO = self.get_preferred_orientations(inh_mod_scale)
                self.POs.append(PO)
    ...
    def get_drift_metrics(self, theta_stim=90):
        ...
        convergence = np.array([initial_distances - distances[:, day] for day in range(len(self.POs) - 1)])
    ...
    def estimate_activity_at_day(self, theta, day, probe_sigma=None):
        ...
        inh_mod_scale = self.inh_scale_timeline[min(day, self.n_days - 1)]
    ...
    def estimate_tuning_curves_at_day(self, day, probe_sigma=None):
        ...
        return tuning_curves_E
    
    def estimate_tuning_curves_over_days(self, probe_sigma=None):
        
        if probe_sigma is None:
            probe_sigma = self.input_sigma

        theta_list = np.linspace(0, 180, self.n_test_angles, endpoint=False)
        self.tuning_curves_over_days = np.zeros((self.n_days + 1, self.N, self.n_test_angles))
        ...
        return self.tuning_curves_over_days
    ...
    def circular_fwhm(self, tuning_curve):
        """FWHM with circular correction for 
        neurons near orientation boundaries."""

        n = len(tuning_curve)
        peak_idx = np.argmax(tuning_curve)
        
        # roll so peak is at center
        shift = n // 2 - peak_idx
        rolled = np.roll(tuning_curve, shift)
        ...
        half_max = peak_val / 2
        above = rolled >= half_max
        transitions = np.where(np.diff(above.astype(int)))[0]
        
        if len(transitions) < 2:
            return np.nan  # never crosses half-max (flat or all-on)
        
        fwhm_bins = transitions[-1] - transitions[0]
        return fwhm_bins * (180.0 / n)

    def estimate_tuning_widths_at_day(self, day, probe_sigma=None, method='circular'):

        if probe_sigma is None:
            probe_sigma = self.input_sigma
        
        tuning_curves_E = self.estimate_tuning_curves_at_day(day, probe_sigma=probe_sigma)
        theta_list = np.linspace(0, 180, self.n_test_angles, endpoint=False)
        tuning_widths = np.zeros(self.N)
        for neuron_idx in range(self.N):
            if method == 'circular':
                tuning_widths[neuron_idx] = self.circular_fwhm(tuning_curves_E[neuron_idx, :])
            elif method == 'standard':
                tuning_widths[neuron_idx] = self.FWHM(theta_list, tuning_curves_E[neuron_idx, :])

        return tuning_widths

    def estimate_tuning_widths_over_days(self, probe_sigma=None, method='circular'):
        
        if probe_sigma is None:
            probe_sigma = self.input_sigma

        self.tuning_widths_over_days = np.zeros((self.n_days + 1, self.N))
        for day in range(self.n_days + 1):
            if method == 'circular':
                self.tuning_widths_over_days[day, :] = self.estimate_tuning_widths_at_day(day, probe_sigma=probe_sigma, method='circular')
            elif method == 'standard':
                self.tuning_widths_over_days[day, :] = self.estimate_tuning_widths_at_day(day, probe_sigma=probe_sigma, method='standard')

        return self.tuning_widths_over_days
    ...
    def plot_population_tuning_curves(self, day, offset=0.028, probe_sigma=None, savefig=False):
        ...
        fig, axs = plt.subplots(figsize=(5, 4), dpi=300)
        for neuron_idx in range(0, self.N, 15): # plot every 30th neuron for visibility
            axs.plot(theta_list, tuning_curves[neuron_idx, :] + offset*neuron_idx, marker='o', ms=1, alpha=0.8, color='black')
    ...
    def save_results(self, drift_mag, drift_rate, convergence):
        # save all weights and tuning curves and drift metrics to hdf5 file
        # also save initial parameters as attributes of the file
        with h5.File(self.save_location + "results.h5", "w") as f:
            f.create_dataset("weights", data=self.weights)
            f.create_dataset("tuning_curves", data=self.tuning_curves)
    ...
    def save_parameters(self):
        """save initial parameters to json file for reproducibility"""
        params = {
            ...
            "vars_ii_mean": self.vars_ii_mean,
            "N": self.N,
            "N_inh": self.N_inh,
            "n_days": self.n_days,
        ...
        with open(self.save_location + "parameters.json", "w") as f:
            json.dump(params, f)
    ...
    def run_analysis(self):
        ...
        self.tuning_curves = self.estimate_tuning_curves_over_days()
        ...
        tuning_widths = self.estimate_tuning_widths_over_days(tuning_curves=self.tuning_curves)
        tuning_widths_assigned = self.vars_ef.copy() # assigned tuning widths based on feedforward weights

        self.plot_drift_against_tuning(np.nanmean(drift_mag, axis=0), tuning_widths, savefig=True)
